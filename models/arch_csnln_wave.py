...
import mmedited.models.common.arch_util as arch_util
...
def wave_conv(in_channels, out_channels, kernel_size,stride=1, bias=True):
    return nn.Conv2d(
...
class CrossScaleAttention(nn.Module):
    def __init__(self, channel=256, reduction=2, ksize=3, scale=[2], stride=1, softmax_scale=10, average=True, conv=default_conv, waveconv=wave_conv):
        ...
        self.conv_match_2 = BasicBlock(waveconv, channel, channel//reduction, 1, bn=False, act=nn.PReLU())  
    ...
    def forward(self, input, input_wave):
        _, _, H, W = input.shape
        ...
        for s in self.scale:
            ...
            ref = self.conv_match_2(input_wave)        # [16, 32, 24, 24]
...
if __name__ == "__main__":
    input = torch.rand(4,256,48,48)
    input_wave = torch.rand(4,256,25,25)
    crossattention = CrossScaleAttention()
    y = crossattention(input, input_wave)
    print(y.size())