...
def make_data_loader(spec, tag=''):
    ...
    loader = DataLoader(dataset, batch_size=spec['batch_size'],
        shuffle=(tag == 'train'), num_workers=8, pin_memory=True,collate_fn=dataset.collate_fn)
    return loader
...
def train(train_loader, model, optimizer, \
    ...
    for batch in tqdm(train_loader, leave=False, desc='train'):
        ...
        # curriculum learning
        if epoch > 500:
            bs = batch['inp_s8'].shape[0]
            inp = (batch['inp_s8'] - inp_sub) / inp_div
            sample_coord = batch['sample_coord_s8']
            cell = batch['cell_s8']
        elif epoch > 250:
            bs = batch['inp_s6'].shape[0]
            inp = (batch['inp_s6'] - inp_sub) / inp_div
            sample_coord = batch['sample_coord_s6']
            cell = batch['cell_s6']      
  
        pred = model(inp, sample_coord, cell)

        sample_coord = sample_coord.unsqueeze(2)


        gt = F.grid_sample((batch['gt'] - gt_sub) / gt_div, sample_coord.flip(-1), \
                            mode='nearest', align_corners=False).permute(0, 2, 3, 1)
        gt = gt.reshape(bs, -1, 3)

        if epoch > 500:
            gt = F.grid_sample((batch['gt_s8'] - gt_sub) / gt_div, sample_coord.flip(-1), \
                                mode='nearest', align_corners=False).permute(0, 2, 3, 1)
            gt = gt.reshape(bs, -1, 3)
        elif epoch > 250:
            gt = F.grid_sample((batch['gt_s6'] - gt_sub) / gt_div, sample_coord.flip(-1), \
                                mode='nearest', align_corners=False).permute(0, 2, 3, 1)
            gt = gt.reshape(bs, -1, 3)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--config', default='/root/autodl-tmp/liwt/configs/train-div2k/train_edsr-baseline-liwt.yaml')
    parser.add_argument('--name', default='_train_edsr-liwt')
